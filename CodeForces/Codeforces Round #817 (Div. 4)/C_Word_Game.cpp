...
void solve(){
    ...
    for(auto x:v1)mp[x].push_back(0);
    for(auto x:v2)mp[x].push_back(1);
    for(auto x:v3)mp[x].push_back(2);
    vector<int>ans(3);

    for(auto x:mp){
        if(x.second.size()>2)continue;

        if(x.second.size()==1) ans[x.second[0]]+=3;

        if(x.second.size()==2){ ans[x.second[0]]++; ans[x.second[1]]++;}

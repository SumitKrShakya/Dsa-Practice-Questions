...
long long binary(vector<long long> &arr, long long target){
    long long start = 0;
...
void solve(){
    ...
    inpt(vec);
    vector<long long> bivec= vec;
    sort(all(bivec));
    map<long long,long long>mp;
    map<long long,long long>mpf;
    ...
    for(long long i=0;i<vec.size();i++){
        long long b = binary(bivec, vec[i]*2+1);
        // cout<<"\n"<<i<<" -> "<<b<<endl;
        if(b!=-1 && mp[b]==i && mpf[b]<=1){
            cout<<binary(bivec,b)<<" ";
        }else{
            cout<<b<<" ";
        }
